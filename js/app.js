$(document).ready(() => {
  ...
  function getMovies(searchText) {
    axios
      .get(`https://www.omdbapi.com?apikey=854c3125&s=${searchText}`)
      .then(response => {
  ...
  function getMovie() {
    ...
    axios
      .get(`https://www.omdbapi.com?apikey=854c3125&i=${movieID}`)
      .then(res => {
        let movie = res.data;
        let output = `
			<div class="row">
		...
		<p>${movie.Plot}</p>
		<hr>
		<a target='_blank' class="btn btn-primary" href="https://imdb.com/title/${
      movie.imdbID
    }">Go IMDB</a>
